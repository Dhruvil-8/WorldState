...
class EventCollisionPayload(BaseModel):
    """Payload representing two conflicting/colliding events to be resolved."""

    existing_event_id: UUID
    existing_title: str
    existing_description: str
    existing_severity: float
...
class ConsensusEventResponse(BaseModel):
    """Output of the Consensus Agent resolving conflicting event data."""

    resolved_title: str
    resolved_description: str
    resolved_severity: float
    resolved_confidence: float
    source_count_increment: int = 1
    timeline_entry: str = ""


class CascadingRiskForecast(BaseModel):
    """Deep analysis cascading risk forecast generated by Graph walk reasoning."""

    title: str
    scenario: str
    severity: float = Field(ge=0, le=10, default=5.0)
    confidence: float = Field(ge=0, le=1, default=0.5)
    impacted_entity_names: list[str] = Field(default_factory=list)
    trigger_event_titles: list[str] = Field(default_factory=list)
